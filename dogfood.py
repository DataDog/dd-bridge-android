...
TARGET_RN = "react-native"

REPOSITORIES = {
    TARGET_RN: "dd-sdk-reactnative"
}

FILE_PATH = {
    TARGET_RN: os.path.join("packages", "core", "android", "build.gradle")
}

PREFIX = {
    TARGET_RN: "com.datadoghq:dd-bridge-android:"
}
...
def parse_arguments(args: list) -> Namespace:
    ...
    parser.add_argument("-v", "--version", required=True, help="the version of the Bridge")
...
def git_push_changes(repo: Repo, version: str):
    ...
    repo.index.commit("Update DD Bridge to " + version)
...
def update_dependant(version: str, target: str, gh_token: str) -> int:
    branch_name = "update_bridge_" + version
