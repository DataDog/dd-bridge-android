...
internal class BridgeSdkTest {
    ...
    @Test
    fun `𝕄 initialize native SDK 𝕎 initialize() {native crash report enabled}`(
        ...
        inOrder(mockDatadog) {
            ...
            verify(mockDatadog).addRumGlobalAttributes(bridgeConfiguration.additionalConfig!!)
        }
    ...
    @Test
    fun `𝕄 initialize native SDK 𝕎 initialize() {native crash report disabled}`(
        @Forgery configuration: DdSdkConfiguration
    ) {
        // Given
    ...
    @Test
    fun `𝕄 initialize native SDK 𝕎 initialize() {native crash report null}`(
        @Forgery configuration: DdSdkConfiguration
    ) {
        // Given
    ...
    @Test
    fun `𝕄 initialize native SDK 𝕎 initialize() {additionalConfiguration is null}`(
        @Forgery configuration: DdSdkConfiguration
    ) {
        // Given
        ...
        inOrder(mockDatadog) {
            ...
            verify(mockDatadog).registerRumMonitor(any())
            verify(mockDatadog).addRumGlobalAttributes(eq(emptyMap()))
        }
        assertThat(configCaptor.firstValue)
            .hasField("coreConfig") {
                it.hasFieldEqualTo("needsClearTextHttp", false)
